	...
	*/
	...
	class GridEyeAPI {
		...
		function __construct() {         // Constructor - open DB connection
			$this->db = new mysqli('localhost', 'root', NULL, 'grideye');
		...
		function getTemps() {   //Method to return the values
			...
			$thing = urldecode($_GET['usertimestamp']);
			if(isset($_GET['usertimestamp']) AND validateDate($_GET['usertimestamp']))
			{
				$usertimestamp = $_GET['usertimestamp'];
				$stmt = $this->db->prepare('SELECT * FROM gridtemps1 WHERE timestamp >= "'.$usertimestamp.'"');
			}
			else
			{
